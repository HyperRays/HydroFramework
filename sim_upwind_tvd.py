...
def vanleer(flux,a,b):
    c = a*b
    c_ge_0 = c > 0
    flux[c_ge_0] += 2*c[c_ge_0]/(a[c_ge_0]+b[c_ge_0])
    return flux
...
class Simulation:
    ...
    def calc_F(self, Uinp, axis, sweepc):
        ...
        # right moving waves
        fr = (u1 * c + w) / 2
        fr_left = self.space.data_in_dir(fr, leftidx)
        dfl = (fr - fr_left) / 2
        dfr = self.space.data_in_dir(dfl, rightidx)
        fr = vanleer(fr,dfl,dfr)

        # left moving waves
        fl_curr = (u1 * c - w) / 2
        fl = self.space.data_in_dir(fl_curr, rightidx)
        fl_left = self.space.data_in_dir(fl, leftidx)
        dfl = (fl_left - fl) / 2
    ...
    def sweepy(self, U, sweepc):
        dFdx = self.calc_F(U[[0, 2, 1, 3]], 1, sweepc)[[0, 2, 1, 3]]
        dU = self.space.external - dFdx
        return U + (dU * self.space.cfg.dt * 1 / sweepc)
